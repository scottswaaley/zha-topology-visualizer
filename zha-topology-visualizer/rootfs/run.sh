...
if [ -f /data/options.json ]; then
    ...
    export ZHA_TOOLKIT_TIMEOUT=$(cat /data/options.json | python3 -c "import sys,json; print(json.load(sys.stdin).get('zha_toolkit_timeout', 300))")
    ...
    export ZHA_TOOLKIT_TIMEOUT=300
    AUTO_REFRESH=0
    export DEBUG=false
fi
