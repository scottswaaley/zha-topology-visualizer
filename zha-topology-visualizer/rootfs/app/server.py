...
def initial_refresh():
    """Regenerate (or fetch) the visualization on startup.

    Always regenerate from the latest export on startup so a new add-on version
    or UI change is applied immediately. The per-request mtime cache only skips
    rebuilds *between* exports - it must not let a stale page (generated by an
    older add-on version) survive a code update.
    """
    ...
    if latest:
        ...
        log("Regenerating visualization with current code...")
        try:
            regenerate_html(force=True)
    ...
    # No existing data, trigger a full refresh
